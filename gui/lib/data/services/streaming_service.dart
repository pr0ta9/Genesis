...
enum StreamEventType { updates, messages, custom, persisted, error }
...
class StreamEvent {
  ...
  factory StreamEvent.fromJson(Map<String, dynamic> json) {
    ...
    final data =
        json['data']; // Keep original type - can be Map, List, String, etc.
...
class WorkflowSection {
  ...
  final bool
  isAutoExpanded; // Whether this section should be auto-expanded during streaming
...
class WorkflowState {
  ...
  const WorkflowState({required this.sections, required this.sectionOrder});
...
class StreamingMessage {
  ...
  final bool
  isPersisted; // Flag to indicate if this is the final persisted message
...
class StreamingService {
  ...
  final StreamController<WorkflowState> _workflowController =
      StreamController<WorkflowState>.broadcast();
  final StreamController<String> _chatTitleController =
      StreamController<String>.broadcast();
  final StreamController<List<List<String>>> _pathDiscoveryController =
      StreamController<List<List<String>>>.broadcast();
  final StreamController<List<String>> _chosenPathController =
      StreamController<List<String>>.broadcast();

  // Executor event streams
  final StreamController<ConsoleEntry> _consoleController =
      StreamController<ConsoleEntry>.broadcast();
  final StreamController<ExecutorStepData> _executorStepController =
      StreamController<ExecutorStepData>.broadcast();
  final StreamController<String> _currentToolController =
      StreamController<String>.broadcast();
  final StreamController<Map<String, int>> _executionProgressController =
      StreamController<Map<String, int>>.broadcast();
  final StreamController<String> _finalOutputController =
      StreamController<String>.broadcast();
  ...
  Stream<List<List<String>>> get pathDiscovery =>
      _pathDiscoveryController.stream;
  ...
  Stream<Map<String, int>> get executionProgress =>
      _executionProgressController.stream;
  Stream<String> get finalOutput => _finalOutputController.stream;
  ...
  List<List<String>>?
  _lastPathDiscovery; // Store last emitted paths for late subscribers

  // Track executor state
  final List<ConsoleEntry> _consoleEntries = [];
  String? _currentExecutingTool;
  int? _currentStepIndex;
  final Map<String, String> _stepWorkspaceDirs =
      {}; // tool_name -> workspace_dir
    ...
    try {
      ...
      if (attachments != null && attachments.isNotEmpty) {
        ...
        final request = http.Request('POST', url)
          ...
          ..body =
              'message=${Uri.encodeComponent(message)}&interrupted=${interrupted.toString()}';
      ...
      // Emit initial streaming message so UI displays immediately
      _messageController.add(_currentMessage!);
      debugPrint(
        '🎬 STREAMING: Emitted initial empty message to show UI (id: ${_currentMessage!.id}, isPersisted: ${_currentMessage!.isPersisted})',
      );
  ...
  Future<void> _handleUpdateEvent(StreamEvent event) async {
    ...
    debugPrint('🔄 [UPDATE EVENT] Full data: $data');

    // Ensure data is a Map before processing
    if (data is! Map<String, dynamic>) {
      debugPrint(
        '⚠️ [UPDATE EVENT] Data is not Map<String, dynamic>, skipping',
      );
      return;
    }

    debugPrint(
      '🔄 [UPDATE EVENT] Processing ${data.keys.length} nodes: ${data.keys.join(", ")}',
    );
    ...
    try {
      ...
      final updatedSections = Map<String, WorkflowSection>.from(
        _currentWorkflow?.sections ?? {},
      );
      final currentOrder = List<String>.from(
        _currentWorkflow?.sectionOrder ?? [],
      );
      ...
      for (final entry in data.entries) {
        ...
        if (nodeData.containsKey('${nodeKey}_reasoning')) {
          ...
          debugPrint(
            '🧠 [UPDATE EVENT] Found reasoning for $nodeKey: ${reasoningContent.substring(0, reasoningContent.length > 100 ? 100 : reasoningContent.length)}...',
          );
        } else {
          debugPrint(
            '🧠 [UPDATE EVENT] No reasoning field found for $nodeKey in nodeData keys: ${nodeData.keys.join(", ")}',
          );
        }
        ...
        if (nodeData.containsKey('${nodeKey}_clarification')) {
          ...
          debugPrint(
            '❓ [UPDATE EVENT] Found clarification for $nodeKey: $clarification',
          );
        }
        ...
        if (existing != null) {
          ...
          debugPrint(
            '📝 TITLE: Created and marked "$nodeKey" as done and collapsed',
          );
        }
        ...
        if (nextNode != null && nextNode != 'END' && nextNode.isNotEmpty) {
          ...
          if (nextNodeExisting != null &&
              nextNodeExisting.thinkingTime != null &&
              nextNodeExisting.thinkingTime! > 0) {
            ...
            debugPrint(
              '🔁 TITLE: Loop to "$nextNode" - creating iteration "${nextSectionKey}"',
            );
          } else {
          ...
          if (nextExisting == null) {
            ...
            debugPrint(
              '📝 TITLE: Created next section "${nextSectionKey}" with title "${nextTitle}" (expanded)',
            );
          }
        }
          ...
          if (nodeData.containsKey('all_paths')) {
            ...
            if (allPathsData is List && allPathsData.isNotEmpty) {
              ...
              for (int i = 0; i < allPathsData.length; i++) {
                ...
                if (pathData is List) {
                  ...
                  if (toolNames.isNotEmpty) {
                    ...
                    debugPrint(
                      '🎯 PRECEDENT: Path ${i + 1}: ${toolNames.join(' -> ')}',
                    );
                  }
                }
              }
              ...
              if (allPaths.isNotEmpty) {
                ...
                debugPrint(
                  '🎯 PRECEDENT: Emitted ${allPaths.length} path(s) from precedent',
                );
              }
            }
          }
        ...
        if (nodeKey == 'classify' &&
            nodeData.containsKey('objective') &&
            _currentChatId != null) {
          final objective = nodeData['objective']?.toString() ?? '';
        ...
        if (nodeKey == 'find_path' && nodeData.containsKey('all_paths')) {
          ...
          if (allPathsData is List && allPathsData.isNotEmpty) {
            ...
            for (int i = 0; i < allPathsData.length; i++) {
              ...
              if (pathData is List) {
                ...
                if (toolNames.isNotEmpty) {
                  allPaths.add(toolNames);
                  debugPrint(
                    '🛤️ [UPDATE EVENT] Path ${i + 1}: ${toolNames.join(' -> ')}',
                  );
                }
              }
            }
            ...
            if (allPaths.isNotEmpty) {
              ...
              _pathDiscoveryController.add(allPaths);
              debugPrint(
                '📋 [UPDATE EVENT] FINAL PATHS EMITTED: ${allPaths.length} total paths',
              );
            }
          }
        }
        ...
        if (nodeKey == 'route' && nodeData.containsKey('chosen_path')) {
          ...
          debugPrint(
            '📊 [UPDATE EVENT] chosen_path data type: ${nodeData['chosen_path'].runtimeType}',
          );
          ...
          if (chosenPathData is List) {
            debugPrint(
              '📊 [UPDATE EVENT] chosen_path contains ${chosenPathData.length} tools',
            );
            ...
            if (toolNames.isNotEmpty) {
              ...
              debugPrint(
                '🎯 [UPDATE EVENT] CHOSEN PATH EMITTED: ${toolNames.join(' -> ')}',
              );
              ...
              debugPrint(
                '⚠️ [UPDATE EVENT] Chosen path has no tools, not emitting',
              );
            }
          }
        }
        ...
        if (_currentMessage != null) {
          ...
          // Check if final_output is in the state and emit it
          if (nodeData.containsKey('final_output')) {
            final finalOutputPath = nodeData['final_output']?.toString();
            if (finalOutputPath != null && finalOutputPath.isNotEmpty) {
              debugPrint(
                '🖼️ FINAL OUTPUT: Detected final_output in state: $finalOutputPath',
              );
              _finalOutputController.add(finalOutputPath);
            }
          }
        }
      } // End of for loop
      ...
      if (workflowUpdated) {
        ...
        if (_currentMessage != null) {
          ...
          // Re-emit the message so UI updates with new workflow
          _messageController.add(_currentMessage!);
          debugPrint(
            '🎬 STREAMING: Re-emitted message after workflow update (sections: ${_currentWorkflow!.sections.length}, content: ${_currentMessage!.content.length} chars)',
          );
        }
  ...
  Future<void> _handleMessageEvent(StreamEvent event) async {
    ...
    debugPrint('💬 [MESSAGE EVENT] Received data type: ${data.runtimeType}');
    debugPrint('💬 [MESSAGE EVENT] Raw data: $data');
    ...
    try {
      if (data is List && data.isNotEmpty) {
        ...
        debugPrint(
          '💬 [MESSAGE EVENT] Message chunk type: ${messageChunk.runtimeType}',
        );
        if (messageChunk is Map<String, dynamic>) {
          messageContent = messageChunk['content']?.toString() ?? '';
          debugPrint(
            '💬 [MESSAGE EVENT] Extracted content: "${messageContent.substring(0, messageContent.length > 100 ? 100 : messageContent.length)}..."',
          );
          ...
          if (additionalKwargsRaw is Map<String, dynamic>) {
            reasoningContent =
                additionalKwargsRaw['reasoning_content']?.toString() ?? '';
          }
        }
        ...
        if (data.length > 1 && data[1] is Map<String, dynamic>) {
          ...
          debugPrint('💬 [MESSAGE EVENT] Metadata: $metadata');
          debugPrint(
            '💬 [MESSAGE EVENT] ls_provider: ${metadata['ls_provider']}',
          );
        }
    ...
    debugPrint(
      '💬 [MESSAGE EVENT] Final extracted - content: "${messageContent.isEmpty ? "EMPTY" : messageContent.substring(0, messageContent.length > 50 ? 50 : messageContent.length)}...", reasoning: "${reasoningContent.isEmpty ? "EMPTY" : reasoningContent.substring(0, reasoningContent.length > 50 ? 50 : reasoningContent.length)}...", node: $langgraphNode',
    );

    // Update current message content
    if (_currentMessage != null) {
      debugPrint(
        '📤 [MESSAGE EVENT] Appending content to message: "${messageContent.substring(0, messageContent.length > 100 ? 100 : messageContent.length)}..." (length: ${messageContent.length})',
      );
      _currentMessage = _currentMessage!.copyWith(
        content: _currentMessage!.content + messageContent,
      );
      debugPrint(
        '📤 [MESSAGE EVENT] Total message content length: ${_currentMessage!.content.length}',
      );
    ...
    if (messageContent.isNotEmpty && _currentWorkflow != null) {
      final sections = Map<String, WorkflowSection>.from(
        _currentWorkflow!.sections,
      );
      bool hasAutoExpandedSections = sections.values.any(
        (section) => section.isAutoExpanded,
      );
    ...
    if (reasoningContent.isNotEmpty && langgraphNode != null) {
      ...
      var sections = Map<String, WorkflowSection>.from(
        _currentWorkflow!.sections,
      );
      ...
      final existingKeys = sections.keys
      ...
      if (existingSection == null) {
        ...
        if (!order.contains(sectionKey)) {
          order.add(
            sectionKey,
          ); // Just append, order should be managed by UPDATE events
        }
      } else {
      ...
      // Update message with workflow
      if (_currentMessage != null) {
        _currentMessage = _currentMessage!.copyWith(workflow: _currentWorkflow);
        // Re-emit the message so UI updates with reasoning
        _messageController.add(_currentMessage!);
      }
  ...
  Future<void> _handleCustomEvent(StreamEvent event) async {
    ...
    debugPrint('⚙️ [CUSTOM EVENT] Full data: $data');
    ...
    try {
      if (data is Map<String, dynamic>) {
        ...
        if (toolName != null && status == 'start' && workspaceDir != null) {
          debugPrint(
            '🔧 [CUSTOM EVENT] EXECUTOR START: $toolName at $workspaceDir',
          );
          ...
          debugPrint(
            '🔧 [CUSTOM EVENT] Chosen path for step calc: $chosenPath',
          );
          if (chosenPath != null) {
            for (int i = 0; i < chosenPath.length; i++) {
              final step = chosenPath[i];
              final stepName = step is Map
                  ? step['name']?.toString()
                  : step.toString();
              if (stepName == toolName) {
                stepIndex =
                    i +
                    1; // 1-based indexing for output files (01_tool, 02_tool, etc.)
                debugPrint(
                  '🔧 [CUSTOM EVENT] Found $toolName at index $i, using stepIndex $stepIndex',
                );
                break;
              }
            }
          ...
          debugPrint(
            '🔧 [CUSTOM EVENT] Set _currentStepIndex=$stepIndex, _currentExecutingTool=$toolName',
          );

          // Update workflow to show "Executing..." section
          if (_currentWorkflow != null &&
              !_currentWorkflow!.sections.containsKey('execute')) {
            final updatedSections = Map<String, WorkflowSection>.from(
              _currentWorkflow!.sections,
            );
            ...
            _workflowController.add(_currentWorkflow!);
            debugPrint(
              '🔧 [CUSTOM EVENT] Added "Executing..." section to workflow',
            );
          }
        ...
        // Executor event: Tool execution end
          ...
          debugPrint(
            '✅ [CUSTOM EVENT] Step $stepIndex completed, workspace: $workspaceDir',
          );
        ...
        if (pathData != null && pathData is List) {
          debugPrint(
            '🛤️ PATH DISCOVERY: Found ${pathData.length} tools in path',
          );
          ...
          if (toolNames.isNotEmpty && _pathDiscoveryActive) {
            ...
            final isDuplicate = _discoveredPaths.any(
              (path) => path.join('->') == pathString,
            );
        ...
        final reasoningContent = data['content']?.toString();
        final timestamp = data['timestamp']?.toString();

        debugPrint(
          '🤔 [CUSTOM EVENT] Checking for reasoning event: node=$node, has_duration=${thinkDuration != null}, has_content=${reasoningContent != null}, has_timestamp=${timestamp != null}',
        );
        ...
        if (node != null && thinkDuration != null && _currentWorkflow != null) {
          ...
          debugPrint(
            '⏱️ THINKING TIME: node=$node, time=${thinkingTimeSeconds}s',
          );
          if (reasoningContent != null) {
            debugPrint(
              '🧠 [CUSTOM EVENT] Reasoning content for $node: ${reasoningContent.substring(0, reasoningContent.length > 200 ? 200 : reasoningContent.length)}...',
            );
          }
          ...
          final matchingKeys = sections.keys
              .where((k) => k.startsWith(node))
              .toList();
          ...
          if (matchingKeys.isNotEmpty) {
            ...
            if (existingSection != null) {
              ...
              if (_currentMessage != null) {
                ...
                // Re-emit the message so UI updates with thinking time
                _messageController.add(_currentMessage!);
              }

              _workflowController.add(_currentWorkflow!);
              debugPrint(
                '⏱️ THINKING TIME: Updated section "$sectionKey" with time',
              );
            }
          }
        }
  ...
  String _formatNodeTitle(String node) {
    switch (node) {
      ...
      default:
        ...
        return '${node.split('_').map((word) => word.isEmpty ? word : word[0].toUpperCase() + word.substring(1)).join(' ')}...';
    }
  }
    ...
    if (_currentWorkflow != null) {
      ...
      for (final entry in _currentWorkflow!.sections.entries) {
        updatedSections[entry.key] = entry.value.copyWith(
          isThinking: false,
          isAutoExpanded:
              false, // Ensure all sections are collapsed when persisted
        );
      }

      _currentWorkflow = _currentWorkflow!.copyWith(sections: updatedSections);

      _workflowController.add(_currentWorkflow!);
      debugPrint(
        '✅ PERSISTED: Completed workflow with ${_currentWorkflow!.sections.length} sections',
      );
    }
    ...
    if (_currentMessage != null) {
      _currentMessage = _currentMessage!.copyWith(
        ...
        type: data['message_type']
            ?.toString(), // Include message type for interrupt detection
      );

      _messageController.add(_currentMessage!);
      debugPrint(
        '✅ PERSISTED: Completed message with workflow attached, isPersisted=true, type=${_currentMessage!.type}',
      );
    }
  }
  ...
  List<ConsoleEntry> get currentConsoleEntries =>
