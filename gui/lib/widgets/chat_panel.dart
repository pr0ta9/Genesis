...
class ChatPanel extends StatefulWidget {
  ...
  final Function(int messageId, Map<String, dynamic> stateData)?
  onExecutionSelected;
...
class _ChatPanelState extends State<ChatPanel> {
  ...
  final GlobalKey<ModelSelectorState> _modelSelectorKey =
      GlobalKey<ModelSelectorState>();
  bool _loading = false;
  String? _error;
  ...
  String? _currentFinalOutput; // Path to the final output file
  ...
  StreamSubscription<String>? _finalOutputSubscription;
  ...
  void _setupStreamingListeners() {
    ...
    _finalOutputSubscription?.cancel();

    // Listen to execution progress updates
    _progressSubscription = _streamingService.executionProgress.listen((
      progress,
    ) {
      if (mounted) {
        setState(() {
    ...
    // Listen to final output updates
    _finalOutputSubscription = _streamingService.finalOutput.listen((
      finalOutputPath,
    ) {
      if (mounted) {
        setState(() {
          _currentFinalOutput = finalOutputPath;
          debugPrint('🖼️ CHAT PANEL: Final output received: $finalOutputPath');
        });
      }
    });

    // Listen to streaming messages
    _messageStreamSubscription = _streamingService.messages.listen((
      streamingMessage,
    ) {
      ...
      if (streamingMessage.isPersisted) {
        ...
        final assistantMessage = <String, dynamic>{
          ...
          'type':
              ...
              'response', // Include type for interrupt detection
        };
        ...
        if (streamingMessage.workflow != null) {
          ...
          for (final section in streamingMessage.workflow!.sections.values) {
            workflowSections.add({
              ...
              'think_duration':
                  (section.thinkingTime ?? 0.0) *
                  1000000000.0, // Convert back to nanoseconds
            });
          ...
          debugPrint(
            '📋 FINAL MESSAGE: Added workflow with ${workflowSections.length} sections, type=${assistantMessage['type']}',
          );
        }
        ...
        if (shouldHaveSummaryCard) {
          ...
          debugPrint(
            '📊 FINAL MESSAGE: Marked as has_state=true for summary card',
          );
        }
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) {
            setState(() {
              _messages.add(
                assistantMessage,
              ); // Add to end for chronological order with reverse: false
        ...
        debugPrint(
          '📋 FINAL MESSAGE: Added to permanent list, will stop calling _parseStoredReasoning repeatedly',
        );
      } else {
  ...
  void didUpdateWidget(covariant ChatPanel oldWidget) {
    ...
    if (oldWidget.chatId != widget.chatId) {
      debugPrint(
        '🔄 CHAT PANEL: chatId changed from ${oldWidget.chatId} to ${widget.chatId}',
      );
  ...
  Future<void> _sendMessage(
    ...
    setState(() {
      ...
      // Reset final output preview
      _currentFinalOutput = null;
    });
    ...
    try {
      ...
      if (_selectedModel != null && _selectedModel!.id != _lastUsedModelId) {
        debugPrint(
          '🎯 MODEL CHANGED: Selecting model ${_selectedModel!.id} before sending message',
        );
        try {
          ...
          if (isBedrock) {
            ...
            if (awsRegion == null ||
                ...
                awsSecretKey == null) {
              debugPrint(
                '⚠️ AWS credentials not found when trying to send message',
              );
              setState(() {
      ...
      debugPrint(
        '📤 SENDING MESSAGE: $message with ${attachments.length} attachments',
      );
      ...
      if (_messages.isNotEmpty) {
        ...
        if (lastAiMessage.isNotEmpty && lastAiMessage['type'] == 'question') {
          interrupted = true;
          debugPrint(
            '🔄 DETECTED INTERRUPTED WORKFLOW (last AI message was a question)',
          );
        }
      }
      ...
      if (isAlreadySaved) {
        ...
        if (response['success'] == true) {
          ...
          setState(() {
            ...
            if (messageIndex != -1) {
              _messages[messageIndex]['precedent_id'] =
                  response['precedent_id'];
            }
          });
                      ...
                      child: Center(
                        child: ConstrainedBox(
                          ...
                          child: ListView.separated(
                            ...
                            reverse:
                                false, // Normal chat behavior: scroll top to bottom
                            padding: const EdgeInsets.symmetric(vertical: 24),
                            itemCount:
                                _messages.length +
                                (_currentStreamingMessage != null ? 1 : 0),
                            ...
                            itemBuilder: (context, index) {
                              if (_currentStreamingMessage != null &&
                                ...
                                return Column(
                                  ...
                                  children: [
                                    ...
                                    // Show final output preview if available
                                    if (_currentFinalOutput != null)
                                      Padding(
                                        padding: const EdgeInsets.only(
                                          bottom: 12,
                                        ),
                                        child: _buildFinalOutputPreview(
                                          _currentFinalOutput!,
                                        ),
                                      ),
                              ...
                              if (!isUser && message['reasoning'] != null) {
                                storedWorkflow = _parseStoredReasoning(
                                  message['reasoning'],
                                );
                              }
                              ...
                              return Column(
                                ...
                                children: [
                                  ...
                                  if (!isUser && message['has_state'] == true)
                                    FutureBuilder<Map<String, dynamic>?>(
                                      ...
                                      builder: (context, snapshot) {
                                        debugPrint(
                                          '🎯 SUMMARY CARD: Message ID ${message['id']}, connection state: ${snapshot.connectionState}',
                                        );
                                        ...
                                        if (!snapshot.hasData) {
                                          debugPrint(
                                            '❌ SUMMARY CARD: No data for message ${message['id']}',
                                          );
                                          return const SizedBox.shrink();
                                        }

                                        final stateData = snapshot.data!;
                                        final allPaths = _extractAllPaths(
                                          stateData,
                                        );
                                        final chosenPath = _extractChosenPath(
                                          stateData,
                                        );

                                        debugPrint(
                                          '🎯 SUMMARY CARD: Message ${message['id']} - allPaths: ${allPaths.length}, chosenPath: ${chosenPath?.length ?? 0}',
                                        );

                                        // Only show summary card if we actually have path data (indicates find_path stage completed)
                                        final hasPathData =
                                            allPaths.isNotEmpty ||
                                            (chosenPath?.isNotEmpty ?? false);
                                        if (!hasPathData) {
                                          debugPrint(
                                            '❌ SUMMARY CARD: No path data found for message ${message['id']} - state did not go through find_path',
                                          );
                                          return const SizedBox.shrink();
                                        }

                                        debugPrint(
                                          '✅ SUMMARY CARD: Showing card for message ${message['id']} with ${allPaths.length} paths',
                                        );
                                        ...
                                        return SummaryCard(
                                          ...
                                          onTap: () =>
                                              _selectExecutionPanel(messageId),
                                        );
                                      },
                                    ),
                                  ...
                                  ChatMessage(
                                    ...
                                    isUser: isUser,
                                    isStreaming:
                                        false, // Saved messages are not streaming
                                    attachments: messageAttachments,
                                    chatId: widget.chatId,
                                    isPrecedentSaved:
                                        message['precedent_id'] != null,
                                    onSavePrecedent: () =>
                                        _handleSavePrecedent(message),
                                  ),
                                ],
                              );
                            },
  ...
  Widget _buildFinalOutputPreview(String finalOutputPath) {
    // Normalize the path (convert backslashes to forward slashes)
    final normalizedPath = finalOutputPath.replaceAll('\\', '/');
    ...
    if (['png', 'jpg', 'jpeg', 'gif', 'webp'].contains(ext)) {
      ...
      mimeType = 'audio/$ext';
    } else if (['mp4', 'webm', 'ogg'].contains(ext)) {
      mimeType = 'video/$ext';
    }

    // Create a FileAttachment with the path
    // The path will be resolved by the FilePreview widget using the backend API
    final attachment = FileAttachment(
      filename: filename,
      path: finalOutputPath, // Keep the Docker path for backend resolution
      mimeType: mimeType,
    );

    return Container(
      constraints: const BoxConstraints(maxWidth: 650),
      child: FilePreview(attachment: attachment, isCompact: false),
    );
  }
